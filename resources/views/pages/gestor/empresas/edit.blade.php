...
@section('title', 'Editar Firma')

@section('content')
    <div class="mb-8 pb-6 border-b border-[#e3e3e0]">
        <h1 class="text-xl font-semibold">Editar Firma</h1>
        <p class="text-[#706f6c] text-sm">{{ $empresa->nome_fantasia }}</p>
    </div>

    <form action="{{ route('gestor.empresas.update', $empresa) }}" method="POST"
          class="bg-white rounded-lg shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] p-4 sm:p-6">
        @csrf
        @method('PUT')

        @include('pages.gestor.empresas._form-fields', ['empresa' => $empresa])
